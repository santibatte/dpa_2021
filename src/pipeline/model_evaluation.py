...
##
def f_predicted_labels(y_test, predicted_scores, classif_theshold):
    dfx = pd.DataFrame(y_test)
    dfx["1_prob"] = predicted_scores[:, 1]
    dfx["prob_label"] = dfx["1_prob"].apply(lambda x: 1 if x >= classif_theshold else 0)
...
def model_evaluation(y_test_pickle_loc, test_predict_scores_pickle_loc):
    ...
    ##
    predicted_labels = f_predicted_labels(y_test, predicted_scores, classif_theshold)
    ...
    print("\n++ Precision @k: {}".format(precision_at_k(y_test, predicted_scores[:, 1], reference_at_k)))
    print("\n++ Recall @k: {}\n".format(recall_at_k(y_test,predicted_scores[:, 1], reference_at_k)))
